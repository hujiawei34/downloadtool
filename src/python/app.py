...
from utils.constants import PROJECT_ROOT, CONFIG_FILE
import logging
...
log_path = PROJECT_ROOT / "logs" / "flask.log"
logging.basicConfig(
...
def api_download_file():
    ...
    if mode == "remote":
        ...
        try:
            ...
            logging.error(
                f"远程文件下载失败 host={host} user={username} path={rel_path} error={e}"
            )
    ...
    try:
        ...
        if mode == "remote":
            if not config.get("remote_server_list"):
                ...
                return jsonify(
                    {"default_dir": "~", "error": "No remote servers configured"}
                )
            remote = config["remote_server_list"][0]
...
def test_server_connectivity():
    ...
    try:
        ...
        if password:
            ssh.connect(
                host, port=port, username=username, password=password, timeout=5
            )
        else:
            ssh.connect(
                host,
...
def api_delete_file():
    ...
    if mode == "remote":
        ...
        try:
            ...
            logging.error(
                f"远程文件删除失败 host={host} user={username} path={rel_path} error={e}"
            )
